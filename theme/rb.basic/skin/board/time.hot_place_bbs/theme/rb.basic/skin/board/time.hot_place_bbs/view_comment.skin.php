...
<button type="button" class="cmt_btn"><span class="total"><b>방문후기</b> <?php echo $view['wr_comment']; ?></span><span class="cmt_more"></span></button>
...
<section id="bo_vc">
    ...
    for ($i=0; $i<$cmt_amt; $i++) {
        ...
        */
        ...
        $cmt_sv = $cmt_amt - $i + 1; // 방문후기 헤더 z-index 재설정 ie8 이하 사이드뷰 겹침 문제 해결
    ...
    <?php } ?>
    <?php if ($i == 0) { //방문후기이 없다면 ?><p id="bo_vc_empty">등록된 방문후기이 없습니다.</p><?php } ?>
...
<aside id="bo_vc_w" class="bo_vc_w">
    ...
    <textarea id="wr_content" name="wr_content" maxlength="10000" required title="ㅎ" placeholder="방문후기 내용을 입력해주세요." onkeyup="check_byte('wr_content', 'char_count');" <?php if ($is_guest) { ?>style="padding-bottom:60px !important;"<?php } ?>><?php echo $c_wr_content; ?></textarea>
        ...
        <div class="btn_confirm btn_confirm_cm_wrap">
            <ul class="cm_wrpa_write_left">
                <?php if($board['bo_comment_point'] > 0) { ?>
                    <span class="font-B">방문후기을 작성하시면 <span class="main_color"><?php echo number_format($board['bo_comment_point']); ?>P</span> 를 드려요!</span>
                <?php } else if($board['bo_comment_point'] < 0) { ?>
                    <span class="font-B">방문후기을 작성하시면 <span class="main_color"><?php echo number_format($board['bo_comment_point']); ?>P</span> 가 차감되요!</span>
                <?php } else { ?>
            ...
            <ul class="cm_wrpa_write_right">
                ...
                <span class="btn_confirm_btn_wrap">
                    ...
                    <label for="wr_secret"><span></span>비밀방문후기</label>
                </span>
                <button type="submit" id="btn_submit" class="btn_submit">방문후기등록</button>
            </ul>
            <div class="cb"></div>
        </div>
    </div>
...
function fviewcomment_submit(f)
{
    ...
    if (char_min > 0 || char_max > 0)
    {
        ...
        {
            alert("방문후기은 "+char_min+"글자 이상 쓰셔야 합니다.");
            return false;
        } else if (char_max > 0 && char_max < cnt)
        {
            alert("방문후기은 "+char_max+"글자 이하로 쓰셔야 합니다.");
            return false;
        }
    }
    else if (!document.getElementById('wr_content').value)
    {
        alert("방문후기을 입력하여 주십시오.");
        return false;
    }
...
{
    return confirm("이 방문후기을 삭제하시겠습니까?");
}

comment_box('', 'c'); // 방문후기 입력폼이 보이도록 처리하기위해서 추가 (root님)
