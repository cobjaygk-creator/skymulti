    ...
    <ul class="gallery_top_mt rb_bbs_list swiper-wrapper swiper-wrapper-<?php echo $bo_table ?>">
        ...
        <div class="gallery_v_mtop swiper-slide swiper-slide-<?php echo $bo_table ?>" <?php echo $rb_ev_stl ?>>
            <ul class="gallery-item-img">
                <a href="<?php echo $wr_href ?>" class="view_modal"><?php echo run_replace('thumb_image_tag', $img_content, $thumb); ?></a>
            ...
            <ul class="gallery-item-tit cut2"><a href="<?php echo $wr_href ?>" class="font-B view_modal"><?php echo $list[$i]['subject'] ?></a></ul>
            <ul class="gallery-item-con cut2">
            <?php if (strstr($list[$i]['wr_option'], 'secret')) { ?>
                <?php echo $sec_txt ?>
            <?php } else { ?>
                <a href="<?php echo $wr_href ?>" class="view_modal"><?php echo $wr_content ?></a>
            <?php } ?>
            </ul>
...
.modal_content {
    ...
    margin: 5vh auto; /* 상단 여백 */
    width: 90%;
    max-width: 600px !important; /* 최대 너비 */
    height: 90vh; /* 높이 */
    border-radius: 10px;
...
@media screen and (max-width: 768px) {
    ...
    .close_btn {
        top: 10px;
        right: 15px;
        color: #333; /* 모바일에서는 배경이 흰색일 수 있으므로 잘 보이게 */
...
<div id="board_modal" class="modal_overlay">
    <div class="modal_content">
        <span class="close_btn">&times;</span>
        <iframe id="board_iframe" src="" frameborder="0"></iframe>
    </div>
</div>
...
$(document).ready(function(){
    ...
    $(document).on('click', '.view_modal', function(e){
        ...
        // iframe src 설정 및 모달 열기
        $('#board_iframe').attr('src', url);
        $('#board_modal').fadeIn(200);
        $('body').css('overflow', 'hidden'); 
    });

    // 2. ★ 중요: 아이프레임 로딩 완료 시 스타일 강제 주입 ★
    $('#board_iframe').on('load', function(){
        var $iframeContents = $(this).contents();
        
        var hideStyle = `
            <style>
                /* [숨김 처리] 헤더, 푸터, 비주얼, 카테고리, 검색창 등 */
                #hd, #ft, .header, .footer, #gnb, .tnb, #header, #footer,
                #main_visual_bg,
                #bo_cate, .bo_cate_link, 
                .bo_sch_wrap, fieldset.bo_sch,
                #bo_list_total, .btns_gr_wrap
                { display: none !important; }

                /* [가로 스크롤 제거 및 레이아웃 초기화] */
                html, body {
                ...
                /* [여백 및 사이즈 조정] */
                #wrapper, #container_wr, .container, .inner {
                    width: 100% !important;
                    min-width: 0 !important;
                    max-width: 100% !important;
                    margin: 0 !important;
                    box-shadow: none !important; /* 테마의 그림자 제거 */
                    box-sizing: border-box !important; /* 패딩이 너비에 포함되도록 설정 */
                }

                /* [★ 요청하신 상하좌우 패딩 여백 추가] */
                #wrapper {
                    padding: 30px !important; /* 상하좌우 30px 여백 */
                }

                /* 이미지나 테이블이 튀어나가는 것 방지 */
                img { max-width: 100% !important; height: auto !important; }
                table { max-width: 100% !important; }
            </style>
        `;
        
        $iframeContents.find('head').append(hideStyle);
    });

    // 3. 닫기 버튼(X) 클릭 시 모달 닫기
    $('.close_btn').click(function(){
        $('#board_modal').fadeOut(200, function(){
            $('#board_iframe').attr('src', ''); 
        });
    ...
    $(window).click(function(event) {
        if ($(event.target).is('#board_modal')) {
            $('#board_modal').fadeOut(200, function(){
                $('#board_iframe').attr('src', '');
            });
            $('body').css('overflow', 'auto');
