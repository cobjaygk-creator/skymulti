...
?>
    ...
    <?php
    // wr_1, wr_2에 전체 URL이 들어올 경우 ID만 추출하는 함수
    function extract_youtube_id($url) {
        ...
        // 쇼츠
        if (preg_match('~youtube\.com/shorts/([\w-]{11})~', $url, $matches)) {
            return $matches[1];
        }
        // 이미 ID만 들어온 경우
    ...
    $wr1_id = isset($view['wr_1']) ? extract_youtube_id($view['wr_1']) : '';
    $wr2_id = isset($view['wr_2']) ? extract_youtube_id($view['wr_2']) : '';
    ?>
    <?php if($wr1_id) { ?>
        <div class="video-container youtube_wrap">
            <div class="youtube">
                <img src="https://img.youtube.com/vi/<?php echo $wr1_id; ?>/hqdefault.jpg" alt="유튜브 썸네일" style="max-width:100%;border-radius:8px;margin-bottom:10px;" loading="lazy">
                <iframe id="youtube-iframe" src="https://www.youtube.com/embed/<?php echo $wr1_id; ?>" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            </div>
            <a href="https://www.youtube.com/watch?v=<?php echo $wr1_id; ?>" class="urls font-B" target="_blank">https://www.youtube.com/watch?v=<span class="main_color"><?php echo $wr1_id; ?></span></a>
        </div>
    <?php } ?>
    <?php if($wr2_id) { ?>
        <div class="video-container youtube_shorts_wrap" style="display:flex; justify-content:center; margin:24px 0;">
            <div class="youtube-shorts" style="width:100%; max-width:400px; aspect-ratio:9/16; background:#000; position:relative;">
                <iframe src="https://www.youtube.com/embed/<?php echo $wr2_id; ?>" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen style="width:100%; height:100%; position:absolute; top:0; left:0;"></iframe>
            </div>
        </div>
    <?php } ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const debouncedUpdateIframePosition = debounce(updateIframePosition, 100);
            window.addEventListener('scroll', debouncedUpdateIframePosition);
            window.addEventListener('resize', debouncedUpdateIframePosition);
