...
$loca = isset($_GET['loca']) ? sql_real_escape_string($_GET['loca']) : '';
...
function get_board_sfl_select_options2($sfl){
    ...
    $str .= '<option value="wr_8" '.get_selected($sfl, 'wr_8', true).'>회사명</option>';
...
<?php if(isset($config['cf_kakao_js_apikey']) && $config['cf_kakao_js_apikey']) {  ?>
    ...
    <div id="map" style="width: 100%; height: 30vh; margin:0px; margin-bottom:30px; border-radius:6px;"></div>
    ...
    <script>
        ...
        $ca = isset($_GET['sca']) ? sql_real_escape_string(str_replace("/", "", $_GET['sca'])) : '';
                                                                                 

        if(isset($ca) && $ca) { 
            $sql = " SELECT * FROM {$write_table} WHERE ca_name = '{$ca}' ORDER BY wr_id ASC ";
        } else if(isset($loca) && $loca) { 
            $sql = " SELECT * FROM {$write_table} WHERE wr_9 = '{$loca}' ORDER BY wr_id ASC ";
        } else { 
        ...
        var wr_8 = '<?php echo str_replace("'", "\\'", get_text($row['wr_8'], 1)); ?>';
        ...
        var content = '<div class="wrap">' +
            ...
            '                <?php if(isset($row['wr_8'])) { ?><div class="sub3 cut80">' + wr_8 + '</div><?php } ?>' +
            '            </div>' +
            '        </div>' +
            '    </div>' +
            '</div>';
    ...
    <form name="fboardlist" id="fboardlist" action="<?php echo G5_BBS_URL; ?>/board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
        ...
        <ul class="wrap_selec">
                ...
                <script>
                    ...
                    $("#loca").val("<?php echo get_text($loca); ?>").prop("selected", true);
                </script>
        </ul>
        <?php } ?>
            ...
            <div class="bbs_prd_list">
                ...
                <div class="bbs_prd_list_wrap" onclick="location.href='<?php echo $wr_href ?>';">
                    ...
                    <ul class="bbs_prd_list_con">
                        ...
                        <?php if(isset($list[$i]['wr_8']) && $list[$i]['wr_8']) { ?>
                        <li class="bbs_prd_list_con_li3 cut"><?php echo get_text($list[$i]['wr_8']) ?></li>
                        <?php } ?>

                        <div class="bbs_prd_list_con_li4 mt-10">
                            <?php if(isset($wr_4[1]) && $wr_4[1]) { ?>
                            <li><span class="font-R rc_label1"><?php echo get_text($wr_4[1]) ?></span></li>
                            <?php } else { ?>
                            <li><span class="font-R rc_label1"><?php echo get_text($wr_4[0]) ?></span></li>
                            <?php } ?>

                            <?php if(isset($wr_5[1]) && $wr_5[1]) { ?>
                            <li><span class="font-R rc_label2"><?php echo get_text($wr_5[1]) ?></span></li>
                            <?php } ?>

                            <?php if(isset($wr_5[0]) && $wr_5[0]) { ?>
                            <li><span class="font-B rc_label_txt main_color"><?php echo get_text($wr_5[0]) ?></span></li>
                            <?php } ?>
                        </div>
