...
// ▼▼▼ [추가할 코드] 모달창일 경우 푸터 출력 중지 ▼▼▼
if ((isset($_REQUEST['is_modal']) && $_REQUEST['is_modal'] == '1') || (isset($_REQUEST['_lp']) && $_REQUEST['_lp'] == '1')) {
    return;
}
// ▲▲▲ [추가 끝] ▲▲▲
...
?>
	...
	<!-- MR Script Ver 2.0 -->
	<script async="true" src="//log1.toup.net/mirae_log_chat_common.js?adkey=tizef" charset="UTF-8"></script>
...
<script>
(function() {
    // 1. 페이지별로 위치를 따로 저장하기 위해 URL을 키값으로 사용
    var pageKey = 'scroll_pos_' + location.pathname + location.search;

    // 2. 페이지 로드 완료 시 저장된 위치로 이동
    window.addEventListener("load", function() {
        var savedPos = sessionStorage.getItem(pageKey);
        
        // 저장된 위치가 있고, 0보다 크다면 (맨 위가 아니라면)
        if (savedPos > 0) {
            // 브라우저가 자동으로 스크롤을 건드리지 못하게 설정
            if ('scrollRestoration' in history) {
                history.scrollRestoration = 'manual';
            }
            
            // 0.3초 뒤에 강제로 위치 이동 (메인페이지 슬라이더 로딩 시간 고려)
            setTimeout(function() {
