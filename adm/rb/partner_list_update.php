...
if ($_POST['act_button'] == "선택취소") {
    ...
    for ($i = 0; $i < $count_post_chk; $i++) {
        ...
        if(isset($imb_id) && $imb_id) {
            memo_auto_send('입점사 권한이 취소 되었습니다.', '', $imb_id, "system-msg");
        }
        
    }

} else if ($_POST['act_button'] == "선택반려") {
    ...
    for ($i = 0; $i < $count_post_chk; $i++) {
        ...
        $imb_bank = isset($_POST['mb_bank'][$k]) ? $_POST['mb_bank'][$k] : '';

        
        $sql = "UPDATE {$g5['member_table']} SET mb_ssr = '{$imb_ssr}', mb_bank = '{$imb_bank}' where mb_id = '{$imb_id}' ";
        sql_query($sql);
