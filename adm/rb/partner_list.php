...
<div class="tbl_head01 tbl_wrap" id="itemqalist">
    ...
    <tr style="background-color:#fff;">
        ...
        <td class="" nowrap>
        <input type="text" name="mb_bank[<?php echo $i; ?>]" value="<?php echo $rowss['mb_bank']; ?>" class="tbl_input">
        </td>
        ...
        <td class="td_datetime td_mng" nowrap>
           
            <?php if($rowss['mb_partner'] != 2) { ?>
            <a id="data_btn_y"  href="javascript:add_y('<?php echo $rowss['mb_id']; ?>');" class="btn btn_03" data-type="승인">승인</a>
            <a id="data_btn_b" href="javascript:add_b('<?php echo $rowss['mb_id']; ?>');" class="btn btn_02" data-type="반려">반려</a>
            <?php } else { ?>
            <a id="data_btn_n" href="javascript:add_n('<?php echo $rowss['mb_id']; ?>');" class="btn btn_01" data-type="취소">취소</a>
            <?php } ?>
...
function add_b(mb_id) {

    if(confirm('해당 신청건을 반려처리 합니다. 계속 하시겠습니까?\n반려 처리된 회원은 다시 신청할 수 있으며, 반려 사유는 쪽지로 보내주세요.')) {
...
function add_n(mb_id) {
    ...
    if(confirm('해당 승인건을 취소처리 합니다. 계속 하시겠습니까?\n취소 처리된 회원은 다시 신청할 수 있으며, 레벨 조정이 있는 경우 회원가입시 설정된 레벨로 되돌립니다.\n취소 사유는 쪽지로 보내주세요.')) {
...
function fitemqalist_submit(f)
{
    ...
    if(document.pressed  == "선택반려") {
        if (!is_checked("chk[]")) {
            ...
            if(!confirm("선택된 회원을 일괄 반려처리 합니다.\n반려 처리된 회원은 다시 신청할 수 있으며 반려사유는 쪽지로 보내주세요.\n계속 하시겠습니까?")) {
                return false;
            }
        }
    }
    ...
    if(document.pressed  == "선택취소") {
        if (!is_checked("chk[]")) {
            ...
            if(!confirm("선택된 회원을 일괄 승인취소 처리 합니다.\n승인취소 처리된 회원은 다시 신청할 수 있으며, 취소사유는 쪽지로 보내주세요.\n레벨 조정이 있는 경우 회원가입시 설정된 레벨로 되돌립니다.\n계속 하시겠습니까?")) {
                return false;
            }
        }
    }
    ...
    if(document.pressed  == "선택수정") {
        if (!is_checked("chk[]")) {
            ...
            if(!confirm("선택된 회원의 정보를 일괄 변경합니다.\n계속 하시겠습니까?")) {
                return false;
            }
        }
    }
