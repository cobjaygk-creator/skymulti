...
<div id="drop-zone">
    ...
    <div id="chat-footer">
        <div class="chat_message_div">
            ...
            <input type="file" id="file-input" style="display: none;" accept="<?php
            $accept_types = [];
            foreach ($allowed_extensions as $ext) {
                $ext = strtolower(trim($ext));
                if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'ico'])) {
                    $accept_types[] = 'image/' . ($ext === 'jpg' ? 'jpeg' : $ext);
                } elseif (in_array($ext, ['mp4', 'webm'])) {
                    $accept_types[] = 'video/' . $ext;
                } elseif (in_array($ext, ['mp3', 'm4a'])) {
                    $accept_types[] = 'audio/' . ($ext === 'm4a' ? 'mp4' : $ext);
                } else {
                    $accept_types[] = '.' . $ext;
                }
            }
            echo htmlspecialchars(implode(',', array_unique($accept_types)), ENT_QUOTES);
            ?>">
            <!-- 프로그래스바 추가 -->
