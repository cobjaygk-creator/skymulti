...
$pa_level = isset($pa['pa_level']) ? $pa['pa_level'] : '';
    ...
    if ($mb_id && $mb_partner == 2) { //승인
    ...
    } else if ($mb_id && $mb_partner == 1) { //반려
        $sql = " update {$g5['member_table']} set mb_partner = '0', mb_level = '{$config['cf_register_level']}', mb_partner_add_time = '0000-00-00 00:00:00' where mb_id = '{$mb_id}' ";
        sql_query($sql);
        ...
        memo_auto_send('입점사 권한이 취소 되었습니다.', '', $mb_id, "system-msg");
